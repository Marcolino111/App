document.addEventListener('DOMContentLoaded', () => {
    ...
    const OWM_KEY = '3d60d61f5bbb3650679b09819689ba1a';
    ...
    addCityForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const res = await fetch(
                `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(name)}&appid=${OWM_KEY}&units=metric`
            );
    ...
    async function fetchAndDisplay(cityName) {
        try {
            const [curRes, fcRes] = await Promise.all([
                fetch(`https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(cityName)}&appid=${OWM_KEY}&units=metric&lang=it`),
                fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${encodeURIComponent(cityName)}&appid=${OWM_KEY}&units=metric&lang=it`)
            ]);
